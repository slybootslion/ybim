...
// Add an amount of time to a date. Negative numbers can be passed to subtract time.
  ...
  class LuGanttView {
    constructor (options) {
      this.workSpace = options && options.workSpace ? $(options.workSpace) : $("#workSpace")
      this.task = []
      this.gantt = null
      ...
      Date.workingPeriodResolution = 1;

      this.init()
    }

    init () {
      const place=$("<div>").prop("id","TWGanttArea").css( {padding:0, "overflow-y":"auto", "overflow-x":"hidden","border":"1px solid #e5e5e5",position:"relative"});
      this.workSpace.append(place).addClass("TWGanttWorkSpace");
