...
Page({
  ...
  onLoad(options) {
    ...
    this.getData()
    this.setData({ msgTop: navigationBarHeight() })
  },
  ...
  itemClick(e) {
    ...
    this.setData({ popupShow: true, currentId })
  },

  async handleSubmit() {
    const { workhour, list, currentId, s_date, group_id, builing_id } = this.data
    ...
    list.forEach(item => {
      if (emp_list.includes(item.emp_id)) item.workhour = workhour
      item.checked = false
    })
